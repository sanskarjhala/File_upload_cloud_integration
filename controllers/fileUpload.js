    ...
    try{
        ...
        if(videoFile.size > 5 * 1024 * 1024){
            return res.status(500).json(
                {
                    sucess:false,
                    message:"file to large , upload file less than 5 mb "
                }
            )
        }
        ...
        //file format supported hai
        console.log("Uploading to any");
        const response = await uploadFileToCloudinary(videoFile, "MyFolder");
        console.log(response);
...
//handler for upladoing image file with compression code
